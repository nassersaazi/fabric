<?php
	...
	function recievefile(){
		...
		if($first=="loan_request"){
			...
			if($staffId18=="Accepted" || $staffId18=="Rejected"){
				...
				VALUES('$lamoun','$lmember','$ladId','$staffId18')");
				/*{
					echo "<script>alert('Submited successfully')</script>";
	...
	function createforms(){
		...
		echo "<body>";
				...
				echo "</header>";
			echo "</p>"; 
				echo "<a href='admin.php?action=addLoan' style='text-decoration:none;'><input type='button' value='LOAN REPAYMENT DETAILS' style='width:20%; margin-left:20%; background-color:lightblue;'></a>";
	...
	function showmember(){
		...
		{
			echo "<script>alert('Submited successfully')</script>";
			header("location:admin.php?action=addmember");
							...
							if($lo3=mysqli_query($connect, $resul4))
							{
								...
								for($n=0;$n<$num;++$n){
									...
									if($loss != 0&&$re==$member1&&$amount0==$max){
										...
										{
											echo "<script>alert('Submited successfully')</script>";
											header("location:admin.php?action=addinvest");
										}else{
											echo "<script>alert('Failed')</script>";
											header("location:admin.php?action=addinvest");
										}
	...
	function showLoan(){
		...
		{
		echo "<script>alert('Submited successfully')</script>";
			header("location:admin.php?action=addLoan");
		}else{
		echo "<script>alert('Failed')</script>";
			header("location:admin.php?action=addLoan");
	...
	function addLoan(){
		...
		echo "<html>";
			...
			echo "<body>";
				...
				echo "<form action='admin.php?action=showLoan' method='POST'>";
	...
	function homepage(){
					...
					foreach($textArray as $element){
						...
						if($subArray[0]== "contribution"){
							...
							*/
							...
							echo "<form action='admin.php?action=recievefile' method='POST'>";
								echo "<p>";
		...
		function prin(){
			...
			echo "<!DOCTYPE html>";
			echo "<html>";
				echo "<form action='admin.php?action=createforms' method='POST'>";
